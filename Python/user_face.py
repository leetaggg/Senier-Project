...
class Temp_data_recv(QtCore.QThread):
    ...
    def run(self):
        while Thread_Pause:
            global recv_data
            recv_data = clientSocket.recv(1024)#데이터 수신                #######
        ...
        while True:                                                     # 수정 5/23
            ...
            if ret:
                ...
                if self.score >= 0.93:
                    ...
                    cv2.putText(img, str(round(c)), (10, 50), cv2.FONT_HERSHEY_SIMPLEX,
                    ...
                    if c > 3:
                        ...
                        print(self.DB_Path)
                        try:
                            df = DE.find(img_path=img, db_path=self.DB_Path, enforce_detection=True)
                        except ValueError:
                            df = DE.find(img_path=img, db_path=self.DB_Path, enforce_detection=False)
                            print("Run as original picture because face could not be detected (user_face)")
                        save_img = save_imgone
                        
                        # 마스크 시 얼굴인식
                        if len(df) != 0:
                            img_dis = df.iloc[0][1]     # 거리값
                            if img_dis < dddown:
                                dflist = []
                                all_number = None
                                df_count = 0
                                while len(dflist) < 10 and df_count < len(df):
                                ...
                                re_dflist = dflist
                                re_dflist.reverse()
                                result = list(set(re_dflist))
                                total_list = []
                                for l in result:
                                    ranking = list(filter(lambda x: re_dflist[x] == l, range(len(re_dflist))))
                                    total_list.append(sum(ranking)+1)
                                    

                                list_index = total_list.index(max(total_list))
                                #최종 학번
                                finally_id =result[list_index]
                                
                                
                                #파일 경로 찾기
                                id_str = df[df['identity'].str.contains(str(finally_id))]
                                savefile_path = ""
                                First_Path = os.path.dirname(id_str.iloc[0][0])   
                                for (root, directories, files) in os.walk(First_Path):
                                    for file in files:
                                        if '.jpg' in file:
                                            savefile_path = os.path.join(root, file)
                                            
                                StudentID = finally_id
                                aa = "학번은 : " + str(StudentID)+ " 거리값 : "+ str(round(id_str.iloc[0][1],3)) + "\n" + str(self.DB_Path)
                                test_a(aa)
                                print("학번은 : " + str(StudentID)+ " 거리값 : "+ str(round(id_str.iloc[0][1],3)))
                                try:
                                    cameraimg = img_save(savefile_path)
                                except:
                                    del df
                                #print(cameraimg)
                                #이미지 저장
...
class Ui_user_face(object):
    def __init__(self, day, week, Name):
        global Name_list
        self.daylist = day
        self.weeklist = week
        Name_list = Name
    def setupUi(self, Dialog):
        ...
        self.Temp.setGeometry(QtCore.QRect(10, 500, 210, 40))
        ...
        self.Name.setGeometry(QtCore.QRect(440, 500, 210, 40))
        ...
        self.Student_Name.setGeometry(QtCore.QRect(230, 500, 210, 40))
        font = QtGui.QFont()
        font.setPointSize(15)
        font.setBold(True)
        font.setWeight(75)
    ...
    def retranslateUi(self, Dialog):
        ...
        self.Student_Name.setText(_translate("Dialog", "이름 : None"))
    def updateFrame(self, image):
        self.Video.setPixmap(QtGui.QPixmap.fromImage(image))
    def updateTemp(self, temp):
        self.Temp.setText("체온 :" + " " + temp[0:4] + "℃")
    def updateName(self, name):
        #print("이름 :",name)
        if name == "None":
            self.Name.setText("학번 :" + " " + name)
            self.Student_Name.setText("이름 : None")
        elif name == "unknown":
            self.Name.setText("학번 :" + " " + name)
            self.Student_Name.setText("이름 : unknown")
        else:
            Student_Name = Name_list.index(name)
            self.Name.setText("학번 :" + " " + name)
            self.Student_Name.setText("이름 :" + " " + Name_list[Student_Name + 1])
...
def test_a(aaaa):            # 나중에 지울 것 $$$$$
    with open('C:/AHard/test.txt','a',encoding='UTF-8') as f:
        a =datetime.today()
        b = "시간:" +str(a) +"\n" + str(aaaa)
